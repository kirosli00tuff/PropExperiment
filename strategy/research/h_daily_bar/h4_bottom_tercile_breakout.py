"""H4 -- bottom-tercile prior range, opening-range breakout (Stage D.1f, Family H, class C7).

Specification, FROZEN: reports/stage_d1f_confirmation_list.md section 2.1 A3, row H4.
Condition on day d (complete prior daily bars only): Range[d-1] <= P33.33 of Range over the
60 complete bars before d-1, with np.percentile(ranges, 33.33, method="linear") (H-7); the
lookback is 61 (d-1 plus those 60). Entry: breakout of the 30-minute opening range, 2 micros,
exit at 14:58 CT; the mechanics are in ``_mechanics``. Instrument guard: d-1 only; the
trailing 60 ranges are exempt (NEW-9). Source: Crabel's narrow-range conditioning; no
parameter is taken from any result seen in this program.
...
LABEL = "H4 bottom-tercile prior range, breakout"
NAME = "h4_bottom_tercile_prior_range_breakout"
LOOKBACK = 61
PERCENTILE_Q = 33.33
GUARDED_BARS = 1
ENTRY = "breakout"
...
def condition(prior: Sequence[DailyBar], *, _lookback: int = LOOKBACK) -> Condition | None:
    ...
    values = (Range[d-1] in ticks, P33.33 cut in ticks); None during warm-up."""
...
class H4BottomTercileBreakout(DailyBarStrategy):
