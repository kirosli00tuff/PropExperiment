...
(Task 5): when the user's Windows PC is available (it is not always), ML
training and backtests can run on it as jobs sent from this machine over
SSH, with results pulled back and checked by hash. The ThinkPad stays the
default machine and runs everything whenever the PC is not available. It
then freezes the whole harness with a sha256 manifest and commits it,
after an adversarial Fable max review. It stops there. The next sessions:
K2's screening session (research window only, needs no purchase, runs on
the ThinkPad if the PC is not ready), E.2c (connect and verify the Windows
PC, whenever the user has done the manual setup steps, blocking nothing),
then the ML route's purchase and training (needs the user to fund acct-2),
then the other clusters.
...
- Output:
  ...
  - Timed probes on synthetic data of the real size (about 450,000 rows,
    ...
    to leave at least 0.5 GB of VRAM free. These are the default machine's
    figures. Under V10 (Task 5) E.2c repeats the probes on the Windows PC
    when it is available. Write the measured times, peak memory and
...
The user decided on 2026-09-26 that ML training and backtests (screening
and confirmation runs) may execute on the Windows PC when it is available:
Ryzen 5, 32 GB of RAM, NVIDIA RTX 2060 Super with 8 GB of VRAM, 2 TB SSD
(figures as the user gave them, measured in E.2c). The PC is shared and
not always available, so the ThinkPad stays the default and every job must
also run on it unchanged. The ThinkPad keeps the Claude sessions,
purchases, sealing, reviews and commits, and sends each job over SSH when
the PC is used.
...
- Output:
  ...
  - The V10 amendment, written by the lead as
    ...
    dated 2026-09-26: the user added the Windows PC as an optional machine
    for ML training and backtests. M8's compute text gains it: the ThinkPad
    and its RTX 3050 stay the default, and when the PC runs the LSTM, the
    frozen A-1 rule plans its batch size against the RTX 2060 Super's 8 GB.
    Every job records which machine ran it. No statistical choice in
